...
export async function edit(todo: Todo) {
  try {
    const documentId = todo._id
    // Create a new object without the 'id' property
    const { _id, ...updatedTodo } = todo;
    const updateResult = await todosCollection.updateOne({ _id: new ObjectId(documentId) }, { $set: updatedTodo });

    if (updateResult.modifiedCount === 1) {
      console.log('Document updated successfully');
    } else {
      console.log('Document not found or no changes applied');
    }
  } catch (error) {
    console.error('Error connecting to MongoDB:', error)
  }
...
export async function deleteOne(id: string) {
  try {
    ...
    const deletionResult = await todosCollection.deleteOne({ _id: new ObjectId(documentId) })
